  ...
  module API
    def create_meter_request(new_resource)
      begin
        ...
        headers = generate_headers(new_resource.api_key)
    ...
    def generate_headers(api_key)
      auth = auth_encode(api_key)
      {"Authorization" => "Basic #{auth}", "Content-Type" => "application/json"}
    end

    def auth_encode(api_key)
      auth = Base64.encode64("#{api_key}:").strip
      auth.gsub("\n","")
    end
    ...
    def build_url(new_resource, action)
      ...
      when :create
        meters_url
      when :search
        "#{meters_url}?name=#{new_resource.name}"
      when :meter, :certificates, :delete
        meter_id = get_meter_id(new_resource)
        "#{meters_url}/#{meter_id}"
      when :tags
        meter_id = get_meter_id(new_resource)
        "#{meters_url}/#{meter_id}/tags"
      end
    end
